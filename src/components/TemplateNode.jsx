...
function generatePrompt(template, values) {
  // Replace all {{param}} with their values, or just the param name if no value
...
const TemplateNode = memo(({ data, selected, id }) => {
  ...
  const handleParamChange = (paramName, value) => {
    window.dispatchEvent(
      new CustomEvent("updateTemplateValues", {
        detail: { id, paramName, value },
      }),
    );
  };
      ...
      >
        ...
        <div className="flex items-center justify-between px-3 py-2.5">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            <div
              className={`w-2 h-2 rounded ${colorScheme.accent} shadow-lg flex-shrink-0`}
            />
            <span
  ...
  return (
    <>
      ...
      >
        ...
        {/* Content */}
        <div className="flex-1 px-3 pb-3 min-h-0 overflow-hidden flex flex-col gap-2">
          {/* Parameters */}
          ...
          <div className="flex-1 min-h-0 overflow-hidden">
            <div className="h-full p-2 rounded-lg bg-black/20 border border-white/5 overflow-hidden">
              <div className="h-full text-xs text-zinc-300 overflow-y-auto custom-scrollbar leading-relaxed">
                <HighlightedTemplate
                  template={template}
                  values={values}
...
// Component to render template with highlighted/replaced values
function HighlightedTemplate({ template, values, colorScheme }) {
  const parts = [];
  let lastIndex = 0;
  const regex = /\{\{(\w+)\}\}/g;
  let match;
      ...
      {parts.map((part, i) => {
        ...
        if (part.value) {
          return (
            <span
              key={i}
              className={`px-1 py-0.5 rounded ${colorScheme.bg} ${colorScheme.text} font-medium`}
            >
              {part.value}
            </span>
          );
        ...
        return (
          <span
            key={i}
            className="px-1 py-0.5 rounded bg-zinc-700/50 text-zinc-400 font-mono"
          >
            {`{{${part.name}}}`}
          </span>
        );
